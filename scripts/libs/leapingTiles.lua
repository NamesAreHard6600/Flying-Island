--Custom Lib Designed for the Skylance
...
--DOES check that from == to. We can't jump from one tile to the same tile.
...
function this:move_tiles(from, to)
  ...
  --Find all the bad indexes and removes them (repeat until there's no bad indexes left)
  repeat
    bad_indexes = {}
    for i, point in ipairs(from) do
      local point2 = to[i]
  ...
  function()
    ...
    local leaping_tiles = {}
    for i, point in ipairs(from) do
      --This may be less lines than it was before, but it is hellishly unreadable
      local building = Board:GetTerrain(point) == TERRAIN_BUILDING
      local current_health = building and Board:GetHealth(point) or 0
      local max_health = building and Board:GetMaxHealth(point) or 0
      local pawn_name = building and self.SpawnPawn.."_"..current_health.."_"..max_health or self.SpawnPawn
      --Double check the pawn exists. If it doesn't, default
      --Better than a leaping combat mech
      local pawn = PAWN_FACTORY:CreatePawn(_G[pawn_name] and pawn_name or self.SpawnPawn)

      Board:SetTerrain(point,TERRAIN_ROAD) --For if it's a building
    ...
    --Has to happen after we call get_effect
    ...
    modApi:scheduleHook(800, function()
      ...
      --Add ending effects, kill all tile pawns, place tiles back and add in buildings
      for k, list in ipairs(leaping_tiles) do
        local pawn = Board:GetPawn(list[1]) --id
        local point = pawn:GetSpace()
        effect:AddBounce(point,2)
        effect:AddBurst(point,"Emitter_Crack_Start",DIR_NONE)
        Board:SetCustomTile(point,self.CustomTile)
        Board:RemovePawn(pawn)
        if list[3] ~= 0 then --max health
